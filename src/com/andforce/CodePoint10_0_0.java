...
class CodePoint10_0_0 implements IGrapheme{
    ...
    private int shouldBreak(int start, int[] mid, int end) {
        ...
        int eModifierIndex = all.lastIndexOf(E_Modifier);
        if (eModifierIndex > 1) {
            List<Integer> slice = all.subList(1, eModifierIndex);
            ...
            List<Integer> Ex = Arrays.asList(Extend, E_Base, E_Base_GAZ);
            if (isExtend && !Ex.contains(start)) {
                return Break;
            }
        }
        ...
        // GB12. ^ (RI RI)* RI	?	RI
        // GB13. [^RI] (RI RI)* RI	?	RI
        int rIIndex = all.lastIndexOf(Regional_Indicator);
        if (rIIndex > 0) {
            List<Integer> slice = all.subList(1, rIIndex);
            boolean isRegional_Indicator = true;
            for (Integer integer : slice) {
                if (integer != Regional_Indicator) {
                    isRegional_Indicator = false;
                    break;
                }
            }
            List<Integer> Ex = Arrays.asList(Prepend, Regional_Indicator);
            if (isRegional_Indicator) {
                if (Ex.contains(previous)) {
                    List<Integer> Regional_IndicatorList = new ArrayList<>();
                    for (Integer integer : all) {
                        if (integer == Regional_Indicator) {
                            Regional_IndicatorList.add(integer);
                        }
                    }
                    if (Regional_IndicatorList.size() % 2 == 1) {
        ...
        // GB10. (E_Base | EBG) Extend* ?	E_Modifier
        int previousNonExtendIndex = all.contains(Extend) ? all.lastIndexOf(Extend) - 1 : all.size() - 2;
        if (Arrays.asList(E_Base, E_Base_GAZ).contains(all.get(previousNonExtendIndex))) {
            List<Integer> slice = all.subList(previousNonExtendIndex + 1, all.size() - 1);
            boolean isExtend = true;
            for (Integer integer : slice) {
                if (integer != Extend) {
                    isExtend = false;
            ...
            if (isExtend && next == E_Modifier) {
                return NotBreak;
            }
        }

        // GB11. ZWJ ? (Glue_After_Zwj | EBG)
        if (previous == ZWJ && Arrays.asList(Glue_After_Zwj, E_Base_GAZ).contains(next)) {
            return NotBreak;
        }
    ...
    private int nextBreak(String string, int index) {
        if (index < 0) {
            return 0;
        }
    ...
    private int graphemesCount(String str) {
        int count = 0;
        int index = 0;
        int brk;
        while ((brk = this.nextBreak(str, index)) < str.length()) {
    ...
    private int getGraphemeBreakProperty(int code) {
